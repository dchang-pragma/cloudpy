...
def download_images(img_urls, dest_dir):
  ...
  index_html = open(local_dir+'index.html','w')
  index_html.write('<verbatim>\n')
  index_html.write('<html>\n')
  index_html.write('<body>\n')

  for url in img_urls:
    url_split_result = urllib.parse.urlsplit(url)
    local_file_name = local_dir + url_split_result.path.split("/")[-1]
    print(local_file_name)
    local_file = urllib.request.urlretrieve(url,local_file_name)
    index_html.write('<img src="'+ local_file_name + '">')
  index_html.write('</body>\n')
  index_html.write('</html>\n')
  index_html.close()
  return
...
download_images(img_urls, '/Users/bunmaster/puzzle/')
